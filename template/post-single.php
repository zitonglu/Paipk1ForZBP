...
{php}SF_img1::getPics($hotlist,355,230,4);{/php}
{php}$src=SF_img1::getPicUrlBy($hotlist->Metas->paipk1_teSeTuPian,355,230,4){/php}
...
{else}
  <div class="more-text-box" style="background-image:url({$host}zb_users/theme/{$theme}/include/nopic.jpg)">
{/if}
      <p class="BMT-title">
        <a href="{$hotlist.Category.Url}" title="{$hotlist.Category.Name}">{$hotlist.Category.Name}</a><br><br>近三个月被浏览 {$hotlist.ViewNums}次</p><p class="more-text-title"><a href="{$hotlist.Url}" title="{$hotlist.Title}">{$hotlist.Title}</a>
      </p>
    </div>
  </div>
{/foreach}
